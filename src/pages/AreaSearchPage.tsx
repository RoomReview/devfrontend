...
const HomePage = () => {
  ...
  const filteredBoroughs = useMemo(() => {
    ...
    return boroughs
      .filter((borough) => borough.name.toLowerCase().includes(normalizedQuery))
      .sort((first, second) => {
        const comparison = first.name.localeCompare(second.name, 'en', { sensitivity: 'base' });
        return sortDirection === 'asc' ? comparison : -comparison;
      });
  }, [boroughs, searchQuery, sortDirection]);

  const totalPages = Math.max(Math.ceil(filteredBoroughs.length / pageSize), 1);
  const visibleBoroughs = filteredBoroughs.slice((currentPage - 1) * pageSize, currentPage * pageSize);

  const handleBoroughSearchKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key !== 'Enter') return;

    const normalizedQuery = searchQuery.trim().toLowerCase();
    const matchingBorough = boroughs.find(
      (borough) => borough.name.trim().toLowerCase() === normalizedQuery,
    );

    if (matchingBorough) {
      event.preventDefault();
      navigate(`/borough/${matchingBorough.id}`);
    }
  };
      ...
      <section className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-[96px] py-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12 max-w-4xl mx-auto md:max-w-none">
          <div className="w-full md:w-auto md:flex-1 md:max-w-[612px] mx-auto">
            <div className="relative border border-[#dcd7d7] rounded-[10px] shadow-[1px_1px_24.9px_0px_#e9ebed] bg-white flex items-center px-6 py-3 hover:border-gray-300 transition-colors cursor-text">
              ...
              <input
                ...
                onKeyDown={handleBoroughSearchKeyDown}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              aria-label={`Sort boroughs ${sortDirection === 'asc' ? 'Z to A' : 'A to Z'}`}
            >
              ...
              <span className="text-[#0b0b0b] text-base font-normal flex items-center gap-1">
                Alphabetical ({sortDirection === 'asc' ? 'A → Z' : 'Z → A'})
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-16 justify-items-center">
          {loading ? (
            ...
            <div className="col-span-full text-center text-[#1A2B3C]">
              {searchQuery ? `No boroughs match “${searchQuery}”.` : 'No borough data available yet.'}
            </div>
          )}
        </div>
