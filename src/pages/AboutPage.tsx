...
import LinkedInImage from '@img/LinkIN.png';
...
const ContributorCard = ({ person, founder = false }: { person: Contributor; founder?: boolean }) => (
  <article className="min-w-0">
    <div className={`flex items-center justify-center overflow-hidden rounded-[17px] bg-white shadow-[0_6px_18px_rgba(15,23,42,0.06)] ${founder ? 'aspect-[2/3]' : 'aspect-[1.3/1]'}`}>
      <img
        src={person.image}
        alt={person.name}
        className={`h-full w-full max-h-full max-w-full bg-white object-center ${founder ? 'object-cover' : 'object-contain'}`}
      />
    </div>
    <div className="pt-3">
...
const AboutPage = () => {
  ...
  return (
    <div className="bg-[#F5F3F1] text-[#1A2B3C]">
      ...
      <section className="mx-auto max-w-[1500px] px-4 py-20 sm:px-6 lg:px-8">
        <div className="mb-6">
          <H2 className="text-[2rem] font-black tracking-[-0.05em] text-[#111111] leading-none sm:text-[2.1rem]">
            Contributors
          </H2>
        </div>

        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1.7fr)] lg:items-start">
          <div className="grid grid-cols-2 gap-5">
            {contributors.map((person) => (
              <ContributorCard key={person.name} person={person} founder />
            ))}
          </div>
